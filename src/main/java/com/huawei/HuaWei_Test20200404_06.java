 ...
 * 分析：
 * 明显的DP问题。设dp[i][j]是生产 到 第i个产品第j个零件所需要 总 的时间，
 * 根据题意得到状态转移dp[i][j] = max(dp[i-1][j], dp[i][j-1]) + nums[i][j] (该零件所需要的时间)。
 * 第一行第一列照抄，输出最后一个就对了
 * https://www.nowcoder.com/discuss/377231?order=0&page=1&pos=8&type=0
 * <p>
 * 测试用例1
 * 1 3
 * 3.0
