...
/**
 * 给定字符串(包含大写字符，小写字符，数字，符号)用-相连，按照规则生成一个新串，并输出 规则1：第一个-之前的字符串不变，后面的字符串按照给定数字n进行截取，每n个字符，组成一个新子串，将新子串用-相连
 * 规则2：如果截取的子串中，大写字符的个数>小写字符的个数，则将小写字符转换为大写 规则3：如果截取的子串中，大写字符的个数<小写字符的个数，则将大写字符转换为小写 规则4：如果截取的子串中，大写字符的个数=小写字符的个数，不变
 * <p>
 * 该答案已经验证 OK
 */
