...
/**
 * # 【加密字符串】 --已完成，但是有可能超时 > 有一串未加密的字符串str，通过对字符串的每一个字母改变实现加密，加密方式为在每个字母str[i]偏移特定数组元素a[i]的量，
 * 数组a前三位已经赋值，a0=1，a1=2,a2=4，当i>=3时，数组元素a[i]=ai-1 + ai-2 +ai-3 ， 例如原文abcde加密后为bdgkr，其中偏移量分别是1,2,4,7,13。
 * <p>
 * - 输入：第一行为一个整数，大于等于1小于等于1000，表示有n组测试数据，每组数据包含一行原文字符串，只包含小写字母，长度大于0小于等于50， - 输出：每组测试数据输出一行，表示加密后的字符串，
 * <p>
 * 解答要求：时间200ms，内存64MB，
 * <p>
 * - 举例： 1 xy 输出： ya
