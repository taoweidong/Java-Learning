...
public class TestStreamAPI3 {
	...
	public static void main(String[] args) {
		...
		Map<Integer, String> map = list.stream().collect(HashMap::new,
				(m, v) -> m.put(v.getId(), v.getName()), HashMap::putAll);
		System.out.println(JSON.toJSONString(map));

		Map<Integer, String> map2 = list.stream().collect(Collectors.toMap(Employee::getId,
				Employee::getName, (old, news) -> news, HashMap::new));
		...
		// 收集
		HashSet<String> hashMap = list.stream().map(Employee::getName)
		...
		// 分组
		Map<Integer, List<Employee>> groupEmp = list.stream()
				.collect(Collectors.groupingBy(Employee::getAge));
		System.out.println(JSON.toJSON(groupEmp));

		// 多级分组
		Map<Integer, Map<String, List<Employee>>> groupEmp2 = list.stream()
				.collect(Collectors.groupingBy(Employee::getAge, Collectors.groupingBy((x) -> {
					if (x.getSalary() < 3000) {
						return "码农";
		...
		// 分区
		Map<Boolean, List<Employee>> empBoolean = list.stream()
