...
public class PrintMatrix {
    ...
    public static void main(String[] args) {
        ...
        //魔方旋转一次
        int col = data[0].length;//总列数
        int row = data.length;//总行数
        int[][] newMatrix = new int[col][row];//定义新的数组结构，旋转之后的数组结构
        for (int i = col - 1; i >= 0; i--) {//每一列
            for (int j = 0; j < row; j++) {//每一行
                newMatrix[col - i - 1][j] = data[j][i];//执行旋转逻辑
            }
        }
